...
def _build_interceptor_fn(func, next):
    ...
    def _wrapper():
        if func.__interceptor__(ctx.request.path_info): # path_info.startswith(pattert) == True
            return func(next)  # checkadmin(next)
        else:
            return next()
    return _wrapper
...
def _build_interceptor_chain(last_fn, *interceptors):
    ...
    fn = last_fn
    for f in L:
        fn = _build_interceptor_fn(f, fn)  #_build_interceptor_fn(interceptor,func)
    return fn
...
class WSGIApplication(object):
    ...
    def get_wsgi_application(self, debug=False):
        ...
        # 根据path_info，从保存的路由信息里，找到相应的处理函数
        def fn_route():
